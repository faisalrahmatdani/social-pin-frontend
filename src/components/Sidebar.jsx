...
const isActiveStyle = "flex items-center px-5 gap-3 text-extrabold border-r-2 border-indigo-600 transition-all duration-200 ease-in-out capitalize";
...
const Sidebar = ({ user, closeToggle }) => {
  ...
  return (
    <div className="flex flex-col justify-between bg-white h-full overflow-y-auto min-w-210 hide-scrollbar">
      <div className="flex flex-col">
        ...
        <div className="flex flex-col gap-5 ">
          <NavLink to="/" className={({ isActive }) => (isActive ? isActiveStyle : isNotActiveStyle)} onClick={handleCloseSidebar}>
            <MdHome fontSize={30} className="pb-1" />
            Home
          </NavLink>
      ...
      {user && (
        <Link to={`user-profile/${user._id}`} className="flex my-5 mb-3 gap-2 p-2 items-center bg-indigo-600 text-white rounded-lg shadow-lg mx-3" onClick={handleCloseSidebar}>
